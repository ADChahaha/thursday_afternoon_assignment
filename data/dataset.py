...
class TrainDataset(Dataset):
    def __init__(self, is_train, args, is_inference = False):
        self.is_inference = is_inference
        self.transform = Get_Transforms(args)[0] if is_train else Get_Transforms(args)[1]
        root = args.data_path if is_train else args.eval_data_path

        dataset_list = root.replace(' ', '').split(',')
        num_datasets = len(dataset_list)

        # 这部分针对特定数据集不同类别数量需要特殊处理
        assert args.num_train is not None
        self.data_list = []
        for dataset in dataset_list:
            if is_inference:
                self.data_list.extend(
                    [{"image_path": img_path} for img_path in self.get_image_paths(dataset)]
                )
            else:
                real_list, fake_list = self.get_real_and_fake_lists(dataset)
                self.data_list.extend(real_list + fake_list)
